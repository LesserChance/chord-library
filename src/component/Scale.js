  ...
  return (
    <div className="Scale my-1 has-text-centered">
      <div className="scaleOptions is-inline-block">
        <button className={"button " + ((props.chordPosition === 0) ? 'is-primary' : '')}>
          {(props.scaleChordTypes[0] === Chords['maj']) ? 'I' : 'i'}
        </button>
        <button className={"button " + ((props.chordPosition === 1) ? 'is-primary' : '')}>
          {(props.scaleChordTypes[1] === Chords['maj']) ? 'II' : 'ii'}
        </button>
        <button className={"button " + ((props.chordPosition === 2) ? 'is-primary' : '')}>
          {(props.scaleChordTypes[2] === Chords['maj']) ? 'III' : 'iii'}
        </button>
        <button className={"button " + ((props.chordPosition === 3) ? 'is-primary' : '')}>
          {(props.scaleChordTypes[3] === Chords['maj']) ? 'IV' : 'iv'}
        </button>
        <button className={"button " + ((props.chordPosition === 4) ? 'is-primary' : '')}>
          {(props.scaleChordTypes[4] === Chords['maj']) ? 'V' : 'v'}
        </button>
        <button className={"button " + ((props.chordPosition === 5) ? 'is-primary' : '')}>
          {(props.scaleChordTypes[5] === Chords['maj']) ? 'VI' : 'vi'}
        </button>
        <button className={"button " + ((props.chordPosition === 6) ? 'is-primary' : '')}>
